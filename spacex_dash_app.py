...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.RangeSlider(
                                    id='payload-slider',
                                    min=int(min_payload),  # Convert to int to fix the TypeError
                                    max=int(max_payload),  # Convert to int to fix the TypeError
                                    step=1000,
...
def update_scatter_chart(selected_site, payload_range):
    ...
    if selected_site == 'ALL':
        ...
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class',
                         color='Booster Version Category', title=f'Correlation between Payload and Success for {selected_site}')
